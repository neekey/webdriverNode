...
var Communicate = module.exports = function( host ){
    ...
    // 获取用户设置
    var userOptions = this._host._config[ '_options' ];

    // 设置默认的请求配置项
    this._requestOptions = _.defaults( {
...
var createOptions = Communicate.prototype.createOptions = function( option ){
    ...
    // 加入默认参数
    var newOption = _.defaults( option, this._requestOptions );

    // 为请求path参数添加前缀 /session/... -> /wd/hb/session/...
    // 在windows下面通过Path模块的join方法会自动将/转化为\\，因此需要转回来
    newOption.path = Path.join( REQUEST_PATH_PREFIX, newOption.path ).replace( /\\/g, '/' );
