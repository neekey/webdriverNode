...
Suite.prototype = {
    ...
    getSummary: function (){
        ...
        this.suites.forEach(function ( suite ){
            ...
            var summary = suite.getSummary();

            suiteCount += summary.suite;
            specCount += summary.spec;
            itemCount += summary.item;
            suiteFailure += summary.suiteFailure;
            specFailure += summary.specFailure;
            itemFailure += summary.itemFailure;

            suiteCount++;
            suiteFailure += ( suite.result ? 0 : 1 );
        });

        this.specs.forEach(function ( spec  ){

            var summary = spec.getSummary();

            specCount++;
            specFailure += ( spec.result ? 0 : 1 );
            itemCount += summary.item;
            itemFailure += summary.failure;
        });
...
Spec.prototype = {
    ...
    /**
     * 根据当前数据，计算所有的item数量和失败数
     * @return {Object} { item: 2, failure: 1 }
     */
    getSummary: function (){

        var failCount = 0;
        this.summary.item = this.items.length;
        ...
        this.summary.failure = failCount;
