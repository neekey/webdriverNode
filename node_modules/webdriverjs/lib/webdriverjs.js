 ...
 * @param command
 */
WebdriverJs.addCommand = function ( type, commandName, command ){
    ...
    prop[commandName] = (function (internalCommandName) {
        ...
        return function () {
            ...
            var self = this;
            var newItem = newQueueItem( type, internalCommandName, command, self, arguments );
            self.Queue.currentQueueItem.add( newItem );
...
//    },
    ...
    log: function ( type, obj ){
        ...
        switch( type ){
            ...
            case 'command':
                ...
                var path = obj.path;
                console.log( colors.violet + "COMMAND\t" + colors.reset, method + '\t',  path );
                break;
            ...
            case 'data':
                ...
                if( stringData && stringData !== '{}' && stringData !== '""' ){
                    console.log( colors.brown + "DATA\t" + colors.reset, stringData )
                }
                break;
            ...
            case 'result':
                ...
                var value = obj.value;
                console.log( colors.teal + "RESULT\t" + colors.reset, value );
                break;
            ...
            case 'error':
                ...
                console.log( colors.red + title + '\t' + colors.reset + "\t", msg );
                ...
                if( msg ){
                    console.log( colors.dkgrey, msg, colors.reset + '\n' );
                }
            ...
            case 'screenshotSave':
                ...
                console.log(colors.red + "SAVING SCREENSHOT WITH FILENAME:" + colors.reset);
                console.log(colors.brown + fileName + colors.reset);
                break;
    ...
    customLog: function (){
        ...
        var curQueueItem = this.Queue.currentQueueItem;
        curQueueItem.addCustomLog.apply( curQueueItem, arguments );
    },

    getAllLogs: function (){

        return this.Queue.getLogs();
    },
...
for (var i = 0, ii = protocolFiles.length; i < ii; i++) {
    if (path.extname(protocolFiles[i]) == ".js") {
        ...
        WebdriverJs.addCommand( 'protocol', commandName, protocolCommands[commandName] );
    }
}
...
for (var i = 0, ii = commandFiles.length; i < ii; i++) {
    if (path.extname(commandFiles[i]) == ".js") {
        ...
        WebdriverJs.addCommand( 'command', commandName, commandList[commandName] );
    }
}
...
for (var i = 0, ii = testFiles.length; i < ii; i++) {
    if (path.extname(testFiles[i]) == ".js") {
        ...
        WebdriverJs.addCommand( 'test', commandName, testList[commandName] );
    }
}
...
for (var i = 0, ii = assertFiles.length; i < ii; i++) {
    if (path.extname(assertFiles[i]) == ".js") {
        ...
        WebdriverJs.addCommand( 'assert', commandName, assertList[commandName] );
    }
}
