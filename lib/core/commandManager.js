...
/**
 * The extension that make a file be recognized as command.
 * @type {String}
 */
var COMMAND_EXTENSION = '.js';
...
CommandManager.prototype = {
    ...
    readCommandDirectory: function( commandDir ){
        ...
        DirReader.readDirSync( commandDir, function( err, path, stats, ifDir ){

            if( err ){
                throw new Error( 'An error occurs when reading commands directory: ' + JSON.stringify( err ) );
            }

            // Add command if path is a file.
            if( ifDir === false ){
        ...
        dirSep.forEach(function( seg, index ){
            ...
            if( seg ){
                ...
                if( index < dirLen - 1 ){
                    ...
                    if( Path.extname( seg ) == COMMAND_EXTENSION ){

                        commandName = Path.basename( seg, COMMAND_EXTENSION );
